 ...
 * make more straight line for long nets [TODO][Zuzana] find way how to allow
 * temporary disable zoom/moving to allow copy of the text [TODO][Zuzana]
 * javaskriptova funkce ktera snizi jas a kontrast vsech objektu v svg (vse
 * zasedne, a pak dalsi ukol bude pouzit to se zvyraznovanim...) [TODO][Zuzana]
 * net mouse over style/ net(link) style (chce to i sjednotit nazvoslovi, na
 * net) soucasne spoje jsou moc tenke, neda se na ne najet myssi po najeti
 * stejne nic neni videt protoze ta cervena se strati [TODO][Marek] toolbar
 * [TODO][Marek] select, multi-select [TODO][Marek] component edit dialog
 ...
 * tak neco bude potreba integrace s filebrowserem) [TODO][Marek] add/delete
 * connection /construct external port (shortcut), predstava je takova ze kdyz
 * se klikne na sit tak se jakoby nalinkuje na kurzor a kde se klikne tam se
 * pripoji (i vickrat za sebou), zrusi se to pres ESC a pri kliknuti a drzeni
 * napr ctrl se misto pripojeni vytvori externi port
 * 
...
App.controller('diagramController', function($scope, $http) {
	$scope.redraw = function() {
		$http.get('/hls/connections-data/' + $scope.selectedFile).then(
				function(res) {
					...
					function findPort(node, portIndex, isOutput) {
						...
						if (!pi)
							throw "Component " + node.name
									+ " has not port with index:" + portIndex
									+ "( isOutput:" + isOutput + " )"
						return pi;
					}
	...
	function innerCellRenderer(params) {
		...
		var imageFullUrl = "/static/hls/connections/"+ image + '.png';
	...
	$scope.fileDialog = function() {
		d3.selectAll("#fileDialog")
		  .style({"display":"block"});
		$http.get('/hls/connections-data-ls/' + $scope.rootDir).then(
				function(res) {
					...
					$scope.fileGridOptions.api.setRowData(filesRowData);//.refreshView();
				});
	}
	$scope.fileDialog()
	// $scope.redraw()

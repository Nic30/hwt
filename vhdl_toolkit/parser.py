...
class Parser():
    ...
    def exprFromJson(jExpr, ctx):
        ...
        if lit:
            ...
            if t == 'ID':
                ...
                v = Value.fromVal(int(v), int)
            elif t == 'FLOAT':
                v = Value.fromVal(float(v), float)
            elif t == 'STRING':
                pass
            else:
        ...
        if binOp:
            ...
            return SignalNode.resForOp(Op(operator, [op0, op1]))
    ...
    def genericFromJson(jGeneric, ctx):
        ...
        defaultVal = Param(Parser.exprFromJson(jGeneric['value'], ctx))
        defaultVal.name = name

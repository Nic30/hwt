...
Port * PortParser::visitPort_reference(
	...
	p->variable->name = strdup(
			ctx->port_identifier()->identifier()->getText().c_str());

	auto c = ctx->constant_expression();
	if (c) {
		p->variable->type = VerExprParser::visitConstant_expression(c);
	}
	auto r = ctx->range_expression();
	if (r)
		p->variable->type = VerExprParser::visitRange_expression(r);
...
Port * PortParser::visitPort_identifier(
	...
	v->name =strdup(ctx->identifier()->getText().c_str());
...
std::vector<Port*> *PortParser::visitList_of_variable_port_identifiers(
	...
	for (auto n : ctx->children) {
		auto cec = std::dynamic_pointer_cast<Verilog2001Parser::Constant_expressionContext>(n);
		if (cec) {
			last->variable->value = VerExprParser::visitConstant_expression(cec);
		} else {
			auto pic = std::dynamic_pointer_cast<Verilog2001Parser::Port_identifierContext>(n);
